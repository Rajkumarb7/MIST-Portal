...
const App: React.FC = () => {
  ...
  // Always pass alphabetically-sorted copies to all child components so every
  // dropdown, card grid, and list is consistently A-Z without each component
  ...
  if (!authState.isAuthenticated) {
    return <AuthOverlay onLogin={handleLogin} staff={sortedStaff} clients={sortedClients} onDataLoaded={handleDataLoaded} />;
  }
    ...
    <div className="min-h-screen bg-[#f8fafc] dark:bg-[#0f172a] text-slate-900 dark:text-slate-100 flex">
      ...
      <main className="flex-1 flex flex-col min-w-0 min-h-screen overflow-y-auto pb-20 md:pb-0">
        ...
        <div className="p-4 md:p-10 max-w-7xl mx-auto w-full">
          {activeTab === 'dashboard' && <Dashboard user={authState.user!} entries={entries} clients={sortedClients} staff={sortedStaff} onUpdateEntries={(e) => saveData(undefined, undefined, e)} />}
          ...
          {activeTab === 'clients' && authState.user?.role === UserRole.MANAGER && (
            <ClientManagement clients={sortedClients} onUpdate={(c) => saveData(undefined, c)} />
          )}
          {activeTab === 'timesheet' && (
            <TimesheetManagement
              user={authState.user!}
          ...
          {activeTab === 'reports' && <Reports user={authState.user!} entries={entries} clients={sortedClients} staff={sortedStaff} />}
          {activeTab === 'analytics' && authState.user?.role === UserRole.MANAGER && (
            <Analytics entries={entries} clients={sortedClients} staff={sortedStaff} />
          )}
