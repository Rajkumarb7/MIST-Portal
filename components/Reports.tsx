...
const Reports: React.FC<ReportsProps> = ({ user, entries, clients, staff }) => {
  ...
  const handleExportCSV = () => {
    const exportData = filteredEntries.map(e => ({
      ...
      'End Time': e.endTime,
      Hours: e.hours,
      'Hourly Rate': (Number(e.hours) || 0) > 0 ? ((Number(e.workEarnings) || 0) / Number(e.hours)).toFixed(2) : '0.00',
      KM: e.km,
      'KM Rate': (Number(e.km) || 0) > 0 ? ((Number(e.travelEarnings) || 0) / Number(e.km)).toFixed(2) : '0.00',
